...
#include "entities.h"
#include "rynda/libs/sqlite/sqlite_orm.h"
	...
	for (int v = currentVersion + 1; v <= kLatestVersion; ++v) {
		if (migrations.contains(v)) {
			try {
				...
				RyndaDatabase::moveCurrentDatabase();
...
namespace RyndaDatabase {

void moveCurrentDatabase() {
	const auto time = base::unixtime::now();

	if (QFile::exists("./tdata/ryndadata.db")) {
		QFile::rename("./tdata/ryndadata.db", QString("./tdata/ryndadata_%1.db").arg(time));
	}

	if (QFile::exists("./tdata/ryndadata.db-shm")) {
		QFile::rename("./tdata/ryndadata.db-shm", QString("./tdata/ryndadata_%1.db-shm").arg(time));
	}

	if (QFile::exists("./tdata/ryndadata.db-wal")) {
		QFile::rename("./tdata/ryndadata.db-wal", QString("./tdata/ryndadata_%1.db-wal").arg(time));
	}
}
