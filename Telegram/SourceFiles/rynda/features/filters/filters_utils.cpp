...
void FilterUtils::importFromLink(const QString &link) {
	if (link.isEmpty()) {
		Ui::Toast::Show(tr::rynda_FiltersToastFailFetch(tr::now));
		return;
	}
	...
	connect(
		...
		[=]
		{
			...
			if (jsonString.isNull()) {
				...
				Ui::Toast::Show(tr::rynda_FiltersToastFailImport(tr::now));
...
void FilterUtils::publishFilters() {
	...
	titlePart.setBody("TeleRynda Filters");
	...
	connect(
		...
		[=]
		{
			...
			if (error == QNetworkReply::NoError && location.isValid()) {
				...
				Ui::Toast::Show(tr::rynda_FiltersToastFailPublish(tr::now));
			}
			_reply->deleteLater();
		});
}
...
bool FilterUtils::importFromJson(const QByteArray &json) {
	...
	if (changes == ApplyChanges{}) {
		Ui::Toast::Show(tr::rynda_FiltersToastFailNoChanges(tr::now));
...
QString FilterUtils::exportFilters() {
	...
	const auto filters = RyndaDatabase::getAllRegexFilters();
	...
	const auto excl = RyndaDatabase::getAllFiltersExclusions();
...
ApplyChanges FilterUtils::prepareChanges(const QJsonObject &root) {
	...
	const auto existingFilters = RyndaDatabase::getAllRegexFilters();
	const auto existingExclusions = RyndaDatabase::getAllFiltersExclusions();
...
void FilterUtils::applyChanges(const ApplyChanges &changes) {
	if (!changes.newFilters.empty()) {
		for (const auto &filter : changes.newFilters) {
			RyndaDatabase::addRegexFilter(filter);
		}
	}
