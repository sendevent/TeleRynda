...
#include "rynda_forward.h"
#include "apiwrap.h"
#include "rynda_sync.h"
#include "lang_auto.h"
...
std::pair<QString, QString> stateName(const PeerId &id) {
	...
	QString messagesString = tr::rynda_RyndaForwardStatusSentCount(tr::now,
															   lt_count1,
	...
	QString chunkString = tr::rynda_RyndaForwardStatusChunkCount(tr::now,
															 lt_count1,
	...
	if (state->state == ForwardState::State::Preparing) {
		status = tr::rynda_RyndaForwardStatusPreparing(tr::now);
	} else if (state->state == ForwardState::State::Downloading) {
		return std::make_pair(tr::rynda_RyndaForwardStatusLoadingMedia(tr::now), "");
	} else if (state->state == ForwardState::State::Sending) {
		status = tr::rynda_RyndaForwardStatusForwarding(tr::now);
	} else {
		// ForwardState::State::Finished
		status = tr::rynda_RyndaForwardStatusFinished(tr::now);
	}
...
static Ui::PreparedList prepareMedia(not_null<Main::Session*> session,
	...
	const auto prepare = [&](not_null<Data::Media*> media)
	{
		...
		auto prepared = Ui::PreparedFile(RyndaSync::filePath(session, media));
...
void sendMedia(
	...
	if (const auto document = primaryMedia->document(); document && document->sticker()) {
		RyndaSync::sendStickerSync(session, message, document);
		return;
	}
	...
	if (mediaType == SendMediaType::Round || mediaType == SendMediaType::Audio) {
		...
		if (!failed && data.size()) {
			file.close();
			RyndaSync::sendVoiceSync(session,
								   data,
...
bool isRyndaForwardNeeded(const std::vector<not_null<HistoryItem*>> &items) {
	const auto needRyndaForward = [&](const auto &item)
	{
		return isRyndaForwardNeeded(item);
	};
	return std::ranges::any_of(items, needRyndaForward);
}

bool isRyndaForwardNeeded(not_null<HistoryItem*> item) {
	if (item->isDeleted() || item->isRyndaNoForwards() || item->unsupportedTTL()) {
		return true;
	}
	return false;
}

bool isFullRyndaForwardNeeded(not_null<HistoryItem*> item) {
	return item->from()->isRyndaNoForwards() || item->history()->peer->isRyndaNoForwards();
}

struct ForwardChunk
{
	bool isRyndaForwardNeeded;
...
void intelligentForward(
	...
	auto currentChunk = ForwardChunk({
		.isRyndaForwardNeeded = isRyndaForwardNeeded(items[0]),
		.items = currentArray
	});

	for (const auto &item : items) {
		if (isRyndaForwardNeeded(item) != currentChunk.isRyndaForwardNeeded) {
			...
			currentChunk = ForwardChunk({
				.isRyndaForwardNeeded = isRyndaForwardNeeded(item),
				.items = currentArray
			});
		}
	...
	for (const auto &chunk : chunks) {
		if (chunk.isRyndaForwardNeeded) {
			...
			RyndaSync::forwardMessagesSync(session, chunk.items, action, draft.options);
...
void forwardMessages(
	...
	if (!toBeDownloaded.empty()) {
		...
		RyndaSync::loadDocuments(session, toBeDownloaded);
	}
	...
	for (int i = 0; i < items.size(); i++) {
		...
		if (!mediaDownloadable(item->media())) {
			RyndaSync::sendMessageSync(session, message);
...
} // namespace RyndaFeatures::RyndaForward
