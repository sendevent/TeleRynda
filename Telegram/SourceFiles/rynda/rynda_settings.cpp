...
#include "rynda_worker.h"
#include "features/translator/rynda_translator.h"
#include "window/window_controller.h"

using json = nlohmann::json;

namespace RyndaSettings {

const std::string filename = "tdata/rynda_settings.json";

std::optional<TeleRyndaSettings> settings = std::nullopt;
...
bool ghostModeEnabled_util(const TeleRyndaSettings &settingsUtil) {
	return
...
void postinitialize() {
	...
	if (settings->appIcon == QString("macos")) {
		settings->appIcon = RyndaAssets::APP_WHITE_ICON;
	}
}

TeleRyndaSettings &getInstance() {
	initialize();
...
void load() {
	...
	try {
		...
		try {
			settings = p.get<TeleRyndaSettings>();
		} catch (...) {
			LOG(("TeleRyndaSettings: failed to parse settings file"));
		}
	} catch (...) {
		LOG(("TeleRyndaSettings: failed to read settings file (not json-like)"));
	}
...
TeleRyndaSettings::TeleRyndaSettings() {
	// ~ Ghost essentials
	...
	// ~ Customization
	appIcon = RyndaAssets::APP_WHITE_ICON;
...
void set_ghostModeEnabled(bool val) {
	...
	if (const auto window = Core::App().activeWindow()) {
		if (const auto session = window->maybeSession()) {
			RyndaWorker::markAsOnline(session); // mark as online to get offline instantly
		}
	}
}
...
void set_translationProvider(const QString &val) {
	...
	Rynda::Translator::TranslateManager::currentInstance()->resetCache();
}
