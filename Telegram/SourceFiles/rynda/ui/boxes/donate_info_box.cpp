...
#include "rynda/ui/boxes/donate_info_box.h"

#include <QSvgRenderer>

#include "lang_auto.h"
#include "rynda/utils/rc_manager.h"
...
QImage MakeSupportLogo() {
	...
	auto svg = QSvgRenderer(QString(":/gui/icons/rynda/donates/support_logo.svg"));
	auto image = QImage(
...
void FillDonateInfoBox(not_null<Ui::GenericBox*> box, not_null<Window::SessionController*> controller) {
	...
	const auto str = tr::rynda_SupportBoxMakeDonationInfo(
		tr::now,
		lt_amount1,
	...
	box->verticalLayout()->add(InfoRow(
		box->verticalLayout(),
		&controller->session(),
		tr::rynda_SupportBoxMakeDonationHeader(tr::now),
		str,
		&st::menuIconEarn,
		emojiHelper.context()));
	...
	const TextWithEntities proofText = tr::rynda_SupportBoxSendProofInfo(
		tr::now,
		lt_item,
	...
	box->verticalLayout()->add(InfoRow(
		box->verticalLayout(),
		&controller->session(),
		tr::rynda_SupportBoxSendProofHeader(tr::now),
		proofText,
		&st::menuIconPhoto,
		Core::TextContext({
	...
	box->verticalLayout()->add(InfoRow(
		box->verticalLayout(),
		&controller->session(),
		tr::rynda_SupportBoxReceiveBadgeHeader(tr::now),
		TextWithEntities{
			tr::rynda_SupportBoxReceiveBadgeInfo(tr::now)
		},
