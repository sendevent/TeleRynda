...
void Content::setupContent(
	...
	const auto current = RyndaUiSettings::getMonoFont();
...
RyndaUi::FontSelectorBox::FontSelectorBox(QWidget *, Window::SessionController *controller, Fn<void(QString font)> hook)
	: _controller(controller), _hook(hook) {
}

void RyndaUi::FontSelectorBox::prepare() {
	...
	addLeftButton(tr::rynda_BoxActionReset(),
				  [=]
				  {
					  _hook(qs(""));
	...
	setTitle(tr::rynda_CustomizeFontTitle());
	...
	const auto fonts = RyndaUi::PrepareFonts();
	const auto inner = setInnerWidget(
		object_ptr<RyndaUi::Content>(this, fonts),
		st::boxScroll,
		topContainer->height());
...
void RyndaUi::FontSelectorBox::setupTop(not_null<Ui::VerticalLayout*> container) {
	if (!_controller) {
		return;
	}
}

void RyndaUi::FontSelectorBox::keyPressEvent(QKeyEvent *e) {
...
int RyndaUi::FontSelectorBox::rowsInPage() const {
	return std::max(height() / RyndaUi::Rows::DefaultRowHeight(), 1);
}

void RyndaUi::FontSelectorBox::setInnerFocus() {
	_setInnerFocus();
}

base::binary_guard
RyndaUi::FontSelectorBox::Show(Window::SessionController *controller, const Fn<void(QString font)> hook) {
