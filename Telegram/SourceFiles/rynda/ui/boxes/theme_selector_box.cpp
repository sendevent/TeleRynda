...
void ThemeSelectorBox::setupContent() {
	...
	setTitle(tr::rynda_MessageShotThemeSelectTitle());
	...
	RyndaFeatures::MessageShot::themeChosen(
	) | rpl::start_with_next(
	...
	RyndaFeatures::MessageShot::paletteChosen(
	) | rpl::start_with_next([=](const auto &palette)
							 {
								 _themeNames.fire(tr::rynda_MessageShotThemeDefault(tr::now));
								 _selectedPalette = palette;
							 },
							 lifetime());

	addButton(tr::rynda_MessageShotThemeApply(),
			  [=]
			  {
