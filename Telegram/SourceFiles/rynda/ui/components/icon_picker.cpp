...
#ifdef Q_OS_WIN
#include "rynda/utils/windows_utils.h"
#endif

const QVector<QString> icons{
	RyndaAssets::APP_WHITE_ICON,
	RyndaAssets::APP_BLACK_ICON,
};
...
void applyIcon() {
#ifdef Q_OS_WIN
	RyndaAssets::loadAppIco();
	reloadAppIconFromTaskBar();
#endif

	Window::OverrideApplicationIcon(RyndaAssets::currentAppLogo());
	...
	for (int row = 0; row < rows; row++) {
		...
		for (int i = 0; i < columns; i++) {
			...
			if (const auto cached = cachedIcons.find(iconName); cached != cachedIcons.end()) {
				...
				icon = cachedIcons[iconName] = RyndaAssets::loadPreview(iconName);
			}
			auto opacity = 0.0f;
			if (iconName == wasSelected) {
				opacity = 1.0f - animation.value(1.0f);
			} else if (iconName == RyndaAssets::currentAppLogoName()) {
		...
		for (int i = 0; i < columns; i++) {
			...
			if (x >= xOffset && x <= xOffset + st::cpIconSize && e->pos().y() >= yOffset
				...
				if (settings.appIcon != iconName) {
					...
					RyndaSettings::set_appIcon(iconName);
					changed = true;
					break;
				}
			}
