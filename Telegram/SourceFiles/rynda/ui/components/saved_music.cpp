...
#include "rynda/ui/utils/color_utils.h"
#include "rynda/ui/utils/itunes_search.h"
#include "rynda/ui/utils/palette.h"
...
QRgb AdjustHsl(QRgb color, float luminance, float saturation = -1.0f) {
	auto hsl = Rynda::Ui::ColorUtils::colorToHSL(color);
	...
	return Rynda::Ui::ColorUtils::HSLToRGB(hsl);
}
...
QPixmap MakeNoCoverImage(const QSize &size) {
	...
	auto image = QImage(size, QImage::Format_ARGB32);
	{
		...
		auto svgIcon = QSvgRenderer(u":/gui/icons/rynda/nocover.svg"_q);
...
QRgb ExtractColorFromCover(const QPixmap &cover) {
	const auto palette = Rynda::Ui::Palette::from(cover).generate();
	...
	const auto contrast = Rynda::Ui::ColorUtils::calculateContrast(whiteColor, extractedColor);
	...
	if (Rynda::Ui::ColorUtils::calculateContrast(whiteColor, adjustedColor) < 3.0f) {
...
RyndaMusicButton::RyndaMusicButton(
	QWidget *parent,
...
RyndaMusicButton::~RyndaMusicButton() = default;

void RyndaMusicButton::updateData(MusicButtonData data) {
...
void RyndaMusicButton::downloadAndMakeCover(FullMsgId msgId) {
...
void RyndaMusicButton::makeCover() {
	...
	crl::async([=, mediaView = _mediaView]
	{
		...
		if (cover.noCover) {
			const auto pix = Rynda::Ui::Itunes::FetchCover(_performerText, _titleText, size);
			if (!pix.isNull()) {
...
void RyndaMusicButton::paintEvent(QPaintEvent *e) {
	if (!_currentCover) {
		return;
	}
...
int RyndaMusicButton::resizeGetHeight(int newWidth) {
