...
#include "rynda/ui/context_menu/menu_item_subtext.h"
#include "rynda/utils/qt_key_modifiers_extended.h"
...
#include "rynda/ui/message_history/history_section.h"
	...
	return [=]
	{
		if (!controller->showFrozenError()) {
			controller->show(Ui::MakeConfirmBox({
				.text = tr::rynda_DeleteOwnMessagesConfirmation(tr::now),
				.confirmed =
				[=](Fn<void()> &&close)
				{
...
void AddDeletedMessagesActions(PeerData *peerData,
	...
	// const auto has = RyndaMessages::hasDeletedMessages(peerData, topicId);
	// if (!has) {
	// 	return;
	// }

	addCallback(
		tr::rynda_ViewDeletedMenuText(tr::now),
		[=]
		{
...
void AddJumpToBeginningAction(PeerData *peerData,
	...
	addCallback(
		tr::rynda_JumpToBeginning(tr::now),
		[=]
		{
			if (user) {
...
void AddShadowBanAction(PeerData *peerData,
	...
	addCallback({
		.text = (shadowBanned
					 ? tr::rynda_FiltersQuickUnshadowBan(tr::now)
					 : tr::rynda_FiltersQuickShadowBan(tr::now)),
...
void AddDeleteOwnMessagesAction(PeerData *peerData,
	...
	addCallback(
		tr::rynda_DeleteOwnMessages(tr::now),
...
void AddHistoryAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	const auto has = RyndaMessages::hasRevisions(item);
	if (!has) {
		return;
	}

	menu->addAction(
		tr::rynda_EditsHistoryMenuText(tr::now),
		[=]
		{
...
void AddHideMessageAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	menu->addAction(
		tr::rynda_ContextHideMessage(tr::now),
		[=]()
		{
...
void AddUserMessagesAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	if (item->history()->peer->isChat() || item->history()->peer->isMegagroup()) {
		menu->addAction(
			tr::rynda_UserMessagesMenuText(tr::now),
			[=]
			{
...
void AddMessageDetailsAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	callback(Window::PeerMenuCallback::Args{
		.text = tr::rynda_MessageDetailsPC(tr::now),
		.handler = nullptr,
		...
		.fillSubmenu = [&](not_null<Ui::PopupMenu*> menu2)
		{
			if (hasAnyPostField) {
				if (!messageViews.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						...
						tr::rynda_MessageDetailsViewsPC(tr::now),
						messageViews
					));
				}
				...
				if (!messageForwards.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						...
						tr::rynda_MessageDetailsSharesPC(tr::now),
						messageForwards
					));
				}
			...
			menu2->addAction(Ui::ContextActionWithSubText(
				menu2->menu(),
				st::menuIconSchedule,
				tr::rynda_MessageDetailsDatePC(tr::now),
			...
			if (view && view->displayedEditDate()) {
				menu2->addAction(Ui::ContextActionWithSubText(
					menu2->menu(),
					st::menuIconEdit,
					tr::rynda_MessageDetailsEditedDatePC(tr::now),
			...
			if (isForwarded) {
				menu2->addAction(Ui::ContextActionWithSubText(
					menu2->menu(),
					st::menuIconTTL,
					tr::rynda_MessageDetailsForwardedDatePC(tr::now),
			...
			if (media && hasAnyMediaField) {
				...
				if (!mediaSize.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::menuIconDownload,
						tr::rynda_MessageDetailsFileSizePC(tr::now),
						mediaSize
					));
				}
				...
				if (!mediaMime.isEmpty()) {
					...
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::menuIconShowAll,
						tr::rynda_MessageDetailsMimeTypePC(tr::now),
						mime.name()
					));
				}
				...
				if (!mediaName.isEmpty()) {
					...
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::ryndaEditsHistoryIcon,
						tr::rynda_MessageDetailsFileNamePC(tr::now),
						shortified,
						[=]
						{
				...
				if (!mediaResolution.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::menuIconStats,
						tr::rynda_MessageDetailsResolutionPC(tr::now),
						mediaResolution
					));
				}
				...
				if (!mediaDC.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::menuIconBoosts,
						tr::rynda_MessageDetailsDatacenterPC(tr::now),
						mediaDC
					));
				}
...
void AddRepeatMessageAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	menu->addAction(
		...
		[=]
		{
			...
			if (RyndaForward::isFullRyndaForwardNeeded(item)) {
				crl::async([=]
				{
					RyndaForward::forwardMessages(session, action, false, resolvedDraft);
				});
			} else if (RyndaForward::isRyndaForwardNeeded(item)) {
				crl::async([=]
				{
					RyndaForward::intelligentForward(session, action, resolvedDraft);
				});
			} else {
...
void AddReadUntilAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	menu->addAction(
		tr::rynda_ReadUntilMenuText(tr::now),
		[=]()
		{
			readHistory(item);
...
void AddBurnAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	menu->addAction(
		tr::rynda_ExpireMediaContextMenuText(tr::now),
		[=]()
		{
			const auto ids = MTP_vector<MTPint>(1, MTP_int(item->id));
			const auto callback = [=]()
...
void AddCreateFilterAction(not_null<Ui::PopupMenu*> menu,
	...
	menu->addAction(
		tr::rynda_RegexFilterQuickAdd(tr::now),
		[=]
		{
			RegexFilter filter;
