...
#include "rynda/ui/message_history/history_inner.h"
...
void InnerWidget::preloadMore(Direction direction) {
	...
	std::vector<RyndaMessageBase> messages;
	if (_item) {
		// viewing edited history
		messages = RyndaMessages::getEditedMessages(_item, minId, maxId, perPage);
	} else {
		// viewing deleted messages
		messages = RyndaMessages::getDeletedMessages(_peer, _topicId, minId, maxId, perPage);
	}
...
void InnerWidget::addMessages(Direction direction, const std::vector<RyndaMessageBase> &messages) {
