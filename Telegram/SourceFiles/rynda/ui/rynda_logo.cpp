...
void loadAppIco() {
	...
	f.close();
	QFile::copy(qsl(":/gui/art/rynda/%1/app_icon.ico").arg(settings.appIcon), tempIconPath);
}
...
QImage CreateImage(const QString &name, const QSize resultImageSize, const int padding = 0) {
	...
	const auto pngPath = qsl(":/gui/art/rynda/%1/app.png").arg(name);
	...
	const auto svgPath = qsl(":/gui/art/rynda/%1/app.svg").arg(name);
