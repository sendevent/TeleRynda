...
void RegexEditBuilder(
	...
	if (filter) {
		box->setTitle(tr::rynda_RegexFiltersEdit());
		data = *filter;
	} else {
		box->setTitle(tr::rynda_RegexFiltersAdd());
		data.reversed = false;
	}
	...
	const auto caseInsensitive = box->addRow(
		object_ptr<Ui::Checkbox>(
			box,
			tr::rynda_CaseInsensitiveExpression(tr::now),
	...
	const auto reversed = box->addRow(
		object_ptr<Ui::Checkbox>(
			box,
			tr::rynda_ReversedExpression(tr::now),
			data.reversed,
			st::defaultBoxCheckbox),
		st::settingsCheckboxPadding);
	...
	auto saveAndClose = [=, id = data.id]
	{
		...
		crl::async([=]
		{
			RyndaDatabase::addRegexFilter(newFilter);
			...
			crl::on_main([=]
			{
				...
				if (showToast) {
					const auto onClick = [=](const auto &...) mutable
					{
						...
						RyndaDatabase::updateRegexFilter(newFilter);
						FiltersCacheController::rebuildCache();
						RyndaSettings::fire_filtersUpdate();
					...
					Ui::Toast::Show(Ui::Toast::Config{
						.text = tr::rynda_RegexFilterBulletinText(
							tr::now,
