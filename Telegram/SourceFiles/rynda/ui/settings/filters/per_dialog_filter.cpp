...
void PerDialogFiltersListController::prepare() {
	...
	const auto exclusions = RyndaDatabase::getAllFiltersExclusions();
	...
	for (const auto &[id, count] : countsByDialogIds) {
		...
		auto status = QString();
		if (count.filters > 0) {
			status += tr::rynda_RegexFiltersAmount(tr::now, lt_count, count.filters);
			if (count.exclusions > 0) {
				status += ", ";
			}
		}
		if (count.exclusions > 0) {
			status += tr::rynda_RegexFiltersExcludedAmount(tr::now, lt_count, count.exclusions);
		}
...
void PerDialogFiltersListController::rowClicked(not_null<PeerListRow*> peer) {
	...
	_controller->showSettings(RyndaFiltersList::Id());
}
