...
#include "rynda/features/filters/filters_utils.h"
...
rpl::producer<QString> RyndaFiltersList::title() {
	if (shadowBan) {
		return tr::rynda_FiltersShadowBan();
	}
	if (!dialogId.has_value()) {
		return tr::rynda_RegexFiltersShared();
	}
	...
	if (from) {
		...
		res = tr::rynda_RegexFiltersHeader(tr::now) + " (" + QString::number(did) + ")";
	}

	return rpl::single(res);
}

RyndaFiltersList::RyndaFiltersList(
	QWidget *parent,
...
void RyndaFiltersList::checkBeforeClose(Fn<void()> close) {
	_controller->showExclude = true;
	_controller->shadowBan = false;
	close();
}

void RyndaFiltersList::addNewFilter(const RegexFilter &filter, bool exclusion) {
	...
	auto defaultClickHandler = [=, this]() mutable
	{
		...
		_contextMenu->addAction(
			...
			[=]
			{
				...
				RyndaDatabase::updateRegexFilter(*state);
		...
		_contextMenu->addAction(
			...
			[=]
			{
				RyndaDatabase::deleteFilter(state->id);
				RyndaDatabase::deleteExclusionsByFilterId(state->id);
				FiltersCacheController::rebuildCache();
				RyndaSettings::fire_filtersUpdate();
			},
			&st::menuIconDelete);
	...
	auto exclusionsClickHandler = [=, controller = _controller, dialogId = dialogId]() mutable
	{
				...
				└── class Ui::ScrollArea
					└── class QWidget
						└── class Ui::PaddingWrap<class Ui::RpWidget>
							└── class Settings::RyndaFiltersList
		 */
		...
		RyndaDatabase::addRegexExclusion(newExclusion);
		FiltersCacheController::rebuildCache();
		RyndaSettings::fire_filtersUpdate();
	...
	auto deleteExclusionsClickHandler = [=, this]() mutable
	{
		...
		_contextMenu->addAction(
			...
			[=, this]
			{
				...
				RyndaDatabase::deleteExclusion(dialogId.value(), state->id);
				FiltersCacheController::rebuildCache();
				RyndaSettings::fire_filtersUpdate();
			},
			&st::menuIconDelete);
...
void RyndaFiltersList::initializeSharedFilters(
	not_null<Ui::VerticalLayout*> container) {
	if (dialogId.has_value() && _controller->showExclude.has_value() && _controller->showExclude.value()) {
		filters = RyndaDatabase::getByDialogId(dialogId.value());
		exclusions = RyndaDatabase::getExcludedByDialogId(dialogId.value());
	} else {
		filters = RyndaDatabase::getShared();

		// remove shared filters that already excluded for that peer exclusion
		if (dialogId.has_value() && _controller->showExclude.has_value() && !_controller->showExclude.value()) {
			const auto excludedForDialogId = RyndaDatabase::getExcludedByDialogId(dialogId.value());
	...
	if (!filters.empty()) {
		AddSkip(container);
		filtersTitle = AddSubsectionTitle(container, tr::rynda_RegexFiltersHeader());
	...
	if (!exclusions.empty()) {
		...
		excludedTitle = AddSubsectionTitle(container, tr::rynda_RegexFiltersExcluded());
	...
	if (filters.empty() && exclusions.empty()) {
		Ui::AddDividerText(container, tr::rynda_RegexFiltersListEmpty());
	}
}

void RyndaFiltersList::initializeShadowBan(not_null<Ui::VerticalLayout*> container) {
	...
	if (RyndaSettings::getInstance().shadowBanIds.size() > 0) {
		AddSkip(container);
...
void RyndaFiltersList::setupContent(not_null<Window::SessionController*> controller) {
	if (shadowBan) {
