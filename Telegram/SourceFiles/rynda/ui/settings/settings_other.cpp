...
#include "rynda/ui/boxes/donate_qr_box.h"
...
Asset getAsset(const QString &name) {
	...
	QString icon = QString(":/gui/icons/rynda/donates/%1.svg").arg(normalized);
...
rpl::producer<QString> RyndaOther::title() {
	return tr::rynda_CategoryOther();
}

RyndaOther::RyndaOther(
	QWidget *parent,
...
void SetupDonations(not_null<Ui::VerticalLayout*> container, not_null<Window::SessionController*> controller) {
	AddSubsectionTitle(container, tr::rynda_SupportHeader());
	AddDonate(
		AddButtonWithIcon(
			container,
	...
	AddDividerText(container,
				   tr::rynda_SupportDescription2(
					   lt_item,
					   rpl::single(
						   Ui::Text::Link(tr::rynda_SupportDescription1(tr::now), QString("tg://support"))
					   ),
...
void SetupCrashReporting(not_null<Ui::VerticalLayout*> container) {
	...
	AddSkip(container);
	AddSubsectionTitle(container, tr::rynda_CategoryOther());

	AddButtonWithIcon(
		container,
		tr::rynda_CrashReporting(),
		st::settingsButton,
		{&st::menuIconReport}
	)->toggleOn(
		...
		[=](bool enabled)
		{
			RyndaSettings::set_crashReporting(enabled);
			RyndaSettings::save();
		},
		container->lifetime());
	AddSkip(container);
	AddDividerText(container, tr::rynda_CrashReportingDescription());
}
...
void SetupOtherThings(not_null<Ui::VerticalLayout*> container, not_null<Window::SessionController*> controller) {
	...
	AddButtonWithIcon(
		container,
		tr::rynda_ResetSettings(),
		st::settingsButton,
		{&st::menuIconRestore}
	)->setClickedCallback([=]
	{
		controller->show(Ui::MakeConfirmBox({
			.text = tr::rynda_ResetSettingsConfirmation(Ui::Text::RichLangValue),
...
void RyndaOther::setupContent(not_null<Window::SessionController*> controller) {
