	...
	return [=]
	{
		...
		if (isCustomBadge) {
			...
			text = custom.text.isEmpty()
					   ? (isExtera
							  ? tr::rynda_DeveloperPopup(
								  tr::now,
								  lt_item,
								  TextWithEntities{peer->name()},
								  Ui::Text::RichLangValue)
							  : tr::rynda_SupporterPopup(
								  tr::now,
								  lt_item,
								  TextWithEntities{peer->name()},
								  Ui::Text::RichLangValue))
			...
			text = peer->isUser()
					   ? tr::rynda_DeveloperPopup(
						   ...
						   TextWithEntities{peer->name()},
						   Ui::Text::RichLangValue)
					   : tr::rynda_OfficialResourcePopup(
						   tr::now,
						   lt_item,
						   TextWithEntities{peer->name()},
						   Ui::Text::RichLangValue);
		} else if (isSupporter) {
			text = tr::rynda_SupporterPopup(
				tr::now,
				lt_item,
				TextWithEntities{peer->name()},
				Ui::Text::RichLangValue);
...
void MarkAsReadThread(not_null<Data::Thread*> thread) {
	...
	RyndaWorker::markAsOnline(&thread->session());
}
					 ...
					 [=](Fn<void()> finish)
					 {
						 if (const auto channel = history->peer->asChannel()) {
							 return history->session().api().request(MTPchannels_ReadHistory(
								 ...
								 MTP_int(tillId)
							 )).done([=] { RyndaWorker::markAsOnline(&history->session()); }).send();
						 }
						 ...
						 )).done([=](const MTPmessages_AffectedMessages &result)
						 {
							 ...
							 RyndaWorker::markAsOnline(&history->session());
						 }).fail([=]
						 {
						 }).send();
					 });
...
QString formatTTL(int time) {
	if (time == 0x7FFFFFFF) {
		return QString("👀 %1").arg(tr::rynda_OneViewTTL(tr::now));
	}
...
void processMessageDelete(not_null<HistoryItem*> item) {
	if (!isMessageSavable(item)) {
		...
		item->setDeleted();
		RyndaMessages::addDeletedMessage(item);
	}
}
