...
void processLegacy(const QString &appdata, const QString &iconPath) {
	auto shortcut = appdata + "/Microsoft/Internet Explorer/Quick Launch/User Pinned/TaskBar/TeleRynda Desktop.lnk";
	if (!QFile::exists(shortcut)) {
		shortcut = appdata + "/Microsoft/Internet Explorer/Quick Launch/User Pinned/TaskBar/TeleRynda.lnk";
	}
	if (!QFile::exists(shortcut)) {
		return;
	}
...
void processNewShortcuts(const QString &iconPath) {
	...
	const auto shortcut = path + u"TeleRynda Desktop/TeleRynda.lnk"_q;
	const auto native = QDir::toNativeSeparators(path).toStdWString();
...
void reloadAppIconFromTaskBar() {
	...
	QString iconPath = appdata + "/TeleRynda.ico";
